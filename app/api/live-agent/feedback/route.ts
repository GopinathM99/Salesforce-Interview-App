...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = (await request.json().catch(() => null)) as FeedbackCreateBody | null;
    ...
    if (error) {
      console.error("Failed to create live agent feedback", error);
      return NextResponse.json({ error: "Could not save feedback." }, { status: 500 });
    }

    return NextResponse.json({ feedback: data });
  } catch (error) {
    console.error("Live agent feedback error", error);
