...
export const CodeBlock = ({
  ...
  return (
    <CodeBlockContext.Provider value={{ value, onValueChange, data }}>
      <div
        className={cn("size-full overflow-hidden rounded-md border bg-black", className)}
        {...props}
      />
...
export const CodeBlockHeader = ({
  ...
  <div
    className={cn(
      "flex flex-row items-center justify-between border-b border-gray-700 bg-gray-800 p-2",
      className
    )}
    {...props}
  />
...
export const CodeBlockFilename = ({
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 text-center text-red-500">{children}</span>
    </div>
  );
};
...
export const CodeBlockSelectTrigger = ({
  ...
  useEffect(() => {
    if (!arrowStyleAdded && typeof document !== 'undefined') {
      arrowStyleAdded = true;
      const style = document.createElement('style');
      style.id = 'codeblock-select-arrow-styles';
      style.textContent = `
        [data-slot="select-trigger"] svg { color: white !important; fill: none !important; stroke: white !important; }
      `;
...
let arrowStyleAdded = false;

export const CodeBlockSelectValue = ({ className, ...props }: CodeBlockSelectValueProps) => {
  useEffect(() => {
    if (!globalStyleAdded && typeof document !== 'undefined') {
...
export const CodeBlockSelectContent = ({
  ...
  return (
    <SelectContent 
      className={cn("min-w-[300px] p-2", className)} 
      {...props}
    >
      {data.map(children)}
    </SelectContent>
...
export const CodeBlockSelectItem = ({
  ...
  const { value: currentValue } = useContext(CodeBlockContext);
  
  return (
    <SelectItem 
      className={cn("text-sm !py-4 !px-6", className, value === currentValue && "hidden")} 
      style={{ paddingTop: '10px', paddingBottom: '5px', paddingLeft: '10px', paddingRight: '14px' }}
      value={value}
      {...props} 
    />
  );
...
export const CodeBlockCopyButton = ({
  ...
  return (
    <Button
      ...
      {...props}
    >
      {children ?? <Icon className="text-white" size={14} />}
    </Button>
  );
};
