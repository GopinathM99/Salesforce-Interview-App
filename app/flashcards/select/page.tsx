...
export default function FlashcardCategorySelectPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (user) {
          const { data: progressData, error: progressError } = await supabase.rpc("get_flashcard_category_progress");
          if (progressError) {
  ...
  const handleCategorySelect = (category: string) => {
    router.push(`/flashcards?category=${encodeURIComponent(category)}`);
  };
      ...
      <div className="card">
        ...
        >
          ...
          <div className="row" style={{ gap: 8 }}>
            <Link className="btn" href="/flashcards/bookmarks">
              My Bookmarks
            </Link>
        ...
        <p className="muted" style={{ marginBottom: 24 }}>
          Choose a category to start practicing flashcards.
        </p>
        {!user && (
